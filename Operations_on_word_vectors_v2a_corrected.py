...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(1 - np.sum(mu_orth * mu_orth)) * (e_w1B - mu_B) / np.linalg.norm(e_w1 - mu_orth)
    corrected_e_w2B = np.sqrt(1 - np.sum(mu_orth * mu_orth)) * (e_w2B - mu_B) / np.linalg.norm(e_w2 - mu_orth)
