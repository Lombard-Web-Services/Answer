...
def yolo_filter_boxes(boxes, box_confidence, box_class_probs, threshold = .6):
    ...
    ### START CODE HERE
    # Step 1: Compute box scores = confidence * class probabilities
    box_scores = box_confidence * box_class_probs

    # Step 2: Find box classes and corresponding box class scores (max per box)
    box_classes = tf.math.argmax(box_scores, axis=-1)
    box_class_scores = tf.reduce_max(box_scores, axis=-1)

    # Step 3: Create filtering mask for boxes meeting threshold
    filtering_mask = box_class_scores >= threshold

    # Step 4: Apply mask to box_class_scores, boxes, and box_classes
...
def yolo_non_max_suppression(scores, boxes, classes, max_boxes = 10, iou_threshold = 0.5):
    ...
    #### START CODE HERE
        ...
        if tf.shape(scores_label)[0] > 0:
            # Apply non max suppression to this class boxes
    ...
    # Concatenate all indices per class into single tensor
    nms_indices = tf.concat(nms_indices, axis=0)

    # Use gathered indices to filter scores, boxes, and classes
...
def yolo_eval(yolo_outputs, image_shape = (720, 1280), max_boxes=10, score_threshold=.6, iou_threshold=.5):
    ...
    # Use the function `yolo_filter_boxes` you've implemented to perform Score-filtering with a threshold of score_threshold (≈1 line)
