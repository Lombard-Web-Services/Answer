...
def triplet_loss(y_true, y_pred, alpha = 0.2):
    ...
    # Step 3: Compute basic loss as (pos_dist - neg_dist + alpha)
    basic_loss = tf.add(tf.subtract(pos_dist, neg_dist), alpha)
    
    # Step 4: Take the maximum of basic_loss and 0.0, then sum over all examples
...
def verify(image_path, identity, database, model):
    ...
    # Convert encoding and database encoding both to numpy arrays
    encoding_np = encoding.flatten()
    identity_enc_np = database[identity].flatten()

    # Step 2: Compute L2 distance using np.linalg.norm
    dist = np.linalg.norm(encoding_np - identity_enc_np)

    # Step 3: Decide if door opens (distance threshold 0.7)
    if dist < 0.7:
...
def who_is_it(image_path, database, model):
    ...
    for (name, db_enc) in database.items():
        ...
        dist = tf.norm(encoding - db_enc, axis=-1).numpy()
